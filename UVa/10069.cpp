...
int main() {
	...
	while (N--) {
		cin >> X >> Z;
		M = X.length(), K = Z.length();
		vector< vector<BigInteger> > numsubs(K+1, vector<BigInteger>(M+1));
		for (i = 0; i <= K; ++i)
			numsubs[i][0] = 0;
		for (j = 0; j <= M; ++j)
			numsubs[0][j] = 1;
		for (i = 1; i <= K; ++i) {
			for (j = 1; j <= M; ++j) {
				if (X[j-1] == Z[i-1])
					numsubs[i][j] = numsubs[i-1][j-1] + numsubs[i][j-1];
				else
					numsubs[i][j] = numsubs[i][j-1];
			}
		}
		cout << numsubs[K][M] << endl;
	}
